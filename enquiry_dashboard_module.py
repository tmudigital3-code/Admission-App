...
def render_enquiry_dashboard(uploaded_files=None):
    ...
    with enh_col1:
        ...
        if isinstance(filtered_df, pd.DataFrame) and 'Enquiry Type' in filtered_df.columns:
            try:
                enquiry_type_counts = pd.Series(filtered_df['Enquiry Type']).value_counts()
                if not enquiry_type_counts.empty:
    ...
    with enh_col2:
        ...
        if isinstance(filtered_df, pd.DataFrame) and 'Gender' in filtered_df.columns:
            try:
                gender_counts = pd.Series(filtered_df['Gender']).value_counts()
                if not gender_counts.empty:
    ...
    with enh_col3:
        ...
        if isinstance(filtered_df, pd.DataFrame) and 'Allotment Status' in filtered_df.columns:
            try:
        ...
        with adv_col1:
            ...
            if isinstance(filtered_df, pd.DataFrame) and 'Enquiry Date' in filtered_df.columns:
                try:
                    ...
                    if not daily_counts.empty:
                        fig_time_series = px.line(x=daily_counts['Enquiry Date'], y=daily_counts['Count'],
        ...
        with adv_col2:
            # Hourly Analysis
            if isinstance(filtered_df, pd.DataFrame) and 'Hour' in filtered_df.columns:
                try:
                    hour_series = pd.Series(filtered_df['Hour'])
        ...
        if isinstance(filtered_df, pd.DataFrame) and 'College' in filtered_df.columns and 'Specialization' in filtered_df.columns:
            try:
                # Create a pivot table for heatmap using explicit pandas operations
                college_series = pd.Series(filtered_df['College'])
                specialization_series = pd.Series(filtered_df['Specialization'])
                # Create crosstab manually
                crosstab_data = pd.crosstab(college_series, specialization_series)
                if not crosstab_data.empty:
                    fig_heatmap = px.imshow(crosstab_data, 
        ...
        if isinstance(filtered_df, pd.DataFrame) and 'Year' in filtered_df.columns and 'Month' in filtered_df.columns:
            try:
                # Create a scatter plot of enquiries by year and month using explicit pandas operations
                year_series = pd.Series(filtered_df['Year'])
                month_series = pd.Series(filtered_df['Month'])
                # Create a DataFrame for grouping
                temp_df = pd.DataFrame({'Year': year_series, 'Month': month_series})
                # Group and count manually
                grouped = temp_df.groupby(['Year', 'Month']).size()
                ...
                for idx, count in grouped.items():
                    if isinstance(idx, tuple) and len(idx) >= 2:
                        year_list.append(idx[0])
                        month_list.append(idx[1])
