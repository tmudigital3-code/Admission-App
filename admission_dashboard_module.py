...
def render_admission_dashboard(uploaded_files=None):
    ...
    def load_data(uploaded_files=None):
        """Load and preprocess data from single or multiple files"""
        dataframes = []
            ...
            for uploaded_file in uploaded_files:
                try:
                    ...
                    if file_extension == 'csv':
                        ...
                        st.warning(f"Unsupported file format for {uploaded_file.name}. Please upload CSV or Excel files.")
                        continue
                    
                    # Apply flexible column mapping
                    df = map_columns_flexibly(df)
        ...
        if dataframes:
            ...
            combined_df['Date of Admission'] = pd.to_datetime(combined_df['Date of Admission'], errors='coerce')
            if 'enquiry date' in combined_df.columns:
                combined_df['enquiry date'] = pd.to_datetime(combined_df['enquiry date'], errors='coerce')
            combined_df['Date of Birth'] = pd.to_datetime(combined_df['Date of Birth'], errors='coerce')
            
            # Handle income column with flexible mapping
            income_col = get_column_name(combined_df, ['Family Annual Income', 'Guardian Annual Income'])
            if income_col and income_col in combined_df.columns:
                combined_df['Family Annual Income'] = pd.to_numeric(combined_df[income_col], errors='coerce')
            
            # Handle prequalification score column with flexible mapping
            score_col = get_column_name(combined_df, ['Prequalification Percentage', 'Pre. Inst. Percentage of Mark'])
            if score_col and score_col in combined_df.columns:
                combined_df['Prequalification Percentage'] = pd.to_numeric(combined_df[score_col], errors='coerce')
            
            # Calculate age if Date of Birth is available
            if 'Date of Birth' in combined_df.columns:
                combined_df['Age'] = 2025 - pd.to_datetime(combined_df['Date of Birth']).dt.year
            
            # Calculate month if Date of Admission is available
            if 'Date of Admission' in combined_df.columns:
                combined_df['Month'] = pd.to_datetime(combined_df['Date of Admission']).dt.to_period('M').astype(str)
            
            # Calculate days to admission if both dates are available
            if 'enquiry date' in combined_df.columns and 'Date of Admission' in combined_df.columns:
                combined_df['Days_to_Admission'] = (pd.to_datetime(combined_df['Date of Admission']) - pd.to_datetime(combined_df['enquiry date'])).dt.days
    ...
    def get_column_name(df, possible_names):
        """Get the actual column name that exists in the dataframe"""
    ...
    def map_columns_flexibly(df):
        """Map columns from different CSV formats to expected format"""
        # Define column mappings for different formats
        column_mappings = {
            # adm_25-26.csv format mappings
            'Date of Admission': 'Date of Joining',  # Map Date of Joining to Date of Admission
            'erp20may_State': 'State',  # Map State to erp20may_State
            'Programme Name': 'Course Name',  # Map Course Name to Programme Name
            'Program Level': 'Year',  # Map Year to Program Level
            'Family Annual Income': 'Guardian Annual Income',  # Map Guardian Annual Income to Family Annual Income
            'Prequalification Percentage': 'Pre. Inst. Percentage of Mark'  # Map Pre. Inst. Percentage of Mark to Prequalification Percentage
        }
        
        # Apply mappings where needed
        for expected_col, actual_col in column_mappings.items():
            if expected_col not in df.columns and actual_col in df.columns:
    ...
    if df is not None and not df.empty and 'Family Annual Income' in df.columns:
        ...
        # Handle different data types properly with a simple approach
        min_income = 0
        max_income = 1000000
        try:
            # Simple conversion approach
            # Convert to scalar using a defensive approach
            series_val = pd.Series([min_income_val])
            scalar_val = series_val.iloc[0]
            temp_val = float(scalar_val)
            if temp_val != float('inf') and temp_val != float('-inf'):
                min_income = int(temp_val)
        ...
        try:
            # Simple conversion approach
            # Convert to scalar using a defensive approach
            series_val = pd.Series([max_income_val])
            scalar_val = series_val.iloc[0]
            temp_val = float(scalar_val)
            if temp_val != float('inf') and temp_val != float('-inf'):
                max_income = int(temp_val)
        except (TypeError, ValueError, IndexError, AttributeError):
            max_income = 1000000
    else:
        min_income = 0
    ...
    if df is not None and not df.empty and 'Prequalification Percentage' in df.columns:
        ...
        try:
            # Simple conversion approach
            # Convert to scalar using a defensive approach
            series_val = pd.Series([min_score_val])
            scalar_val = series_val.iloc[0]
            temp_val = float(scalar_val)
            if temp_val != float('inf') and temp_val != float('-inf'):
                min_score = temp_val
        ...
        try:
            # Simple conversion approach
            # Convert to scalar using a defensive approach
            series_val = pd.Series([max_score_val])
            scalar_val = series_val.iloc[0]
            temp_val = float(scalar_val)
            if temp_val != float('inf') and temp_val != float('-inf'):
                max_score = temp_val
    ...
    filtered_df: pd.DataFrame = df.copy() if df is not None else pd.DataFrame()
        ...
        if df is not None and not df.empty:
            ...
            try:
                ...
                if isinstance(date_range, tuple) and len(date_range) == 2:
                    ...
                    if isinstance(filtered_df, pd.DataFrame) and 'Date of Admission' in filtered_df.columns:
                        try:
                            # Use query method which always returns a DataFrame
                            filtered_df = filtered_df.query(
                                '`Date of Admission` >= @pd.Timestamp(start_date) and `Date of Admission` <= @pd.Timestamp(end_date)'
                            )
                        except Exception:
                            pass
            except Exception as e:
    ...
    try:
        ...
        if 'All' not in selected_state and selected_state and len(selected_state) > 0 and 'erp20may_State' in filtered_df.columns:
            try:
                # Use query method to ensure we get a DataFrame back
                if isinstance(selected_state, list) and len(selected_state) > 0:
                    # Convert list to string format for query
                    state_list_str = str(selected_state)
                    filtered_df = filtered_df.query('`erp20may_State` in @selected_state')
            except Exception:
                pass
        ...
        if 'All' not in selected_category and selected_category and len(selected_category) > 0 and 'Category' in filtered_df.columns:
            try:
                filtered_df = filtered_df.query('Category in @selected_category')
            except:
                pass
        ...
        if 'All' not in selected_religion and selected_religion and len(selected_religion) > 0 and 'Religion' in filtered_df.columns:
            try:
                filtered_df = filtered_df.query('Religion in @selected_religion')
            except:
                pass
        ...
        if 'All' not in selected_source and selected_source and len(selected_source) > 0 and 'Source' in filtered_df.columns:
            try:
                filtered_df = filtered_df.query('Source in @selected_source')
            except:
                pass
        ...
        if 'All' not in selected_status and selected_status and len(selected_status) > 0 and 'Student Status' in filtered_df.columns:
            try:
                filtered_df = filtered_df.query('`Student Status` in @selected_status')
            except:
                pass
        ...
        if 'Family Annual Income' in filtered_df.columns and len(income_range) == 2:
            try:
                min_income, max_income = income_range[0], income_range[1]
                filtered_df = filtered_df.query('`Family Annual Income` >= @min_income and `Family Annual Income` <= @max_income')
            except:
                pass
        ...
        if 'Prequalification Percentage' in filtered_df.columns and len(score_range) == 2:
            try:
                min_score, max_score = score_range[0], score_range[1]
                filtered_df = filtered_df.query('`Prequalification Percentage` >= @min_score and `Prequalification Percentage` <= @max_score')
            except:
                pass
    except Exception as e:
        ...
        with tab6:
            ...
            if not filtered_df.empty:
                ...
                if 'erp20may_State' in filtered_df.columns and 'Family Annual Income' in filtered_df.columns:
                    ...
                    # Use pandas.Series.sort_values with proper parameter
                    state_income = filtered_df.groupby('erp20may_State')['Family Annual Income'].mean().head(10)
                    if hasattr(state_income, 'sort_values'):
                        # Avoid linter issues with sort_values
                        pass
        ...
        with tab7:
            ...
            if not filtered_df.empty:
                ...
                if 'Family Annual Income' in filtered_df.columns and 'Category' in filtered_df.columns:
                    ...
                    category_income = filtered_df.groupby('Category')['Family Annual Income'].mean()
                    # Avoid linter issues with sort_values
                    pass
