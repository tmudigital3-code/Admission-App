...
def stub_admission_dashboard(uploaded_file=None):
...
def stub_applicant_dashboard(uploaded_files=None):
...
def stub_enquiry_dashboard(uploaded_file=None):
...
st.markdown("""
    ...
    .applicant-section {
        ...
        background: linear-gradient(to right, #e8f3e8 0%, #f8f9fa 100%);
    }
    
    .enquiry-section {
        border-left-color: #fd7e14;
        background: linear-gradient(to right, #fcefdc 0%, #f8f9fa 100%);
    }
    
    .help-section {
        border-left-color: #6f42c1;
        background: linear-gradient(to right, #f0e6f7 0%, #f8f9fa 100%);
    }
...
with st.sidebar:
    st.markdown('<div class="sidebar-section admission-section"><div class="sidebar-title">🏫 Admission Data Upload</div><div class="sidebar-description">Upload your admission CSV files here for immediate analysis</div></div>', unsafe_allow_html=True)
    admission_uploaded_files = st.sidebar.file_uploader("Choose Admission Files", type=['csv', 'xlsx', 'xls'], key="admission_upload", accept_multiple_files=True, help="Upload CSV or Excel files with columns: Date of Admission, enquiry date, Date of Birth, Gender, Category, Religion, Programme Name, Program Level, Student Status, erp20may_State, Source, Family Annual Income, Prequalification Percentage")
    
    st.markdown('<div class="sidebar-section applicant-section"><div class="sidebar-title">🎓 Applicant Data Upload</div><div class="sidebar-description">Upload your applicant CSV files here for detailed profiling</div></div>', unsafe_allow_html=True)
    applicant_uploaded_files = st.sidebar.file_uploader("Choose Applicant Files", type=['csv', 'xlsx', 'xls'], key="applicant_upload", accept_multiple_files=True, help="Upload CSV or Excel files with columns: Allotment Status, Level, Discipline, College")
    
    st.markdown('<div class="sidebar-section enquiry-section"><div class="sidebar-title">📞 Enquiry Data Upload</div><div class="sidebar-description">Upload your enquiry CSV files here for conversion analysis</div></div>', unsafe_allow_html=True)
    enquiry_uploaded_files = st.sidebar.file_uploader("Choose Enquiry Files", type=['csv', 'xlsx', 'xls'], key="enquiry_upload", accept_multiple_files=True, help="Upload CSV or Excel files with enquiry data including Enquiry No., Enquiry Date, and other relevant fields")
    
    st.markdown('<div class="sidebar-section help-section"><div class="sidebar-title">💡 Quick Help</div><div class="sidebar-description">Switch between modules using the tabs above. Each tab will automatically use the data uploaded here.</div></div>', unsafe_allow_html=True)
...
with admission_tab:
    ...
    try:
        ...
        render_admission_dashboard(admission_uploaded_files)
...
with enquiry_tab:
    ...
    try:
        # Pass the uploaded files to the dashboard function
        render_enquiry_dashboard(enquiry_uploaded_files)
    except Exception as e:
